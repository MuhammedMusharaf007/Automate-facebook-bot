...
class Facebooklogin():
    ...
    def login(self):
        ...
        password_element.send_keys(self.password) #Give password as input!
        
        login_button = self.driver.find_element_by_id('loginbutton')
        login_button.click() #Sending mouse click

        time.sleep(2) #loading time 2 seconds, give more if you have a slower connection!
...
if __name__ == '__main__':
    #Enter the login credentials here
    username = input("Enter Email ID: ")
    password = getpass("Enter password: ")
    fb_login = Facebooklogin(email = username, password=password, browser='Chrome')
    fb_login.login()
